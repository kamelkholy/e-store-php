                ...
                <div class="col-md-12">
                    <div style="padding:0;">
                        <div class="content__wrapper">
                            <form method="POST" action="{{route('builds.compare')}}">
                                @csrf
                                <div class="form-group">
                                    <div class="row">
                                        <div class="col-md-5">
                                            <select class="form-control" name="build[1]" id="" required>
                                                @foreach($data as $build)
                                                <option value="{{$build->id}}">{{$build->build_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-5">
                                            <select class="form-control" name="build[2]" id="" required>
                                                @foreach($data as $build)
                                                <option value="{{$build->id}}">{{$build->build_name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-2">
                                            <button class="btn" style="background: #f8770c;">Compare</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
                            <h2 class="text-color text-head-table">Your Builds</h2>

                            <table class="table table-bordered table-striped" style="border: 1px solid #ddd">
                                <thead>
                                    <tr>
                                        <th class="text-center">@sortablelink('build_name', 'Name')</th>
                                        <th class="text-center">@sortablelink('build_access', 'Access')</th>
                                ...
                                <tbody class="text-center">
                                    ...
                                    @foreach($data as $row)
                                    <tr>
                                        <td class="text-center align-middle">{{ $row->build_name }}</td>
                                        <td class="text-center align-middle">{{ $row->build_access }}</td>
                                        <td class="text-center align-middle">
                                            <form action="{{route('builds.destroy', [$row->id])}}" method="POST">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                                <a href="{{route('builds.show', [$row->id])}}" class="btn-sm btn-info" style="padding: .5rem .5rem;">Show</a>
